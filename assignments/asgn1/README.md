...
1. Your team should choose an idea for a software application that uses one or more datasets
   ...
   application. All applications must be written in Java.
...
6. You must select a team leader for the assignment. The team leader is not allowed to
   ...
   - The team leader has ultimate authority to decide any architectural, scheduling,
     workload distribution, and other questions that arise
   ...
   - The team leader is responsible for ensuring fairness in the development workloads
   ...
   - The team leader must proactively track all issues submitted on GitHub (discussed
     below) and document how they are handled
   ...
   - The team leader is responsible for ensuring that key questions regarding the specification
     receive answers from the team that produced the specification
...
10. When team members discover issues with other team members' code (or critical code that
    ...
    simply instruct the reporting team member to add the required method and close the
    issue. 
    ...
    failure to resolve the issue and simply reassign the work to another team member. The
